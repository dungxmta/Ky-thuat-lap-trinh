...
void Try(int i, int n, int C[][STP],  int daxet[], int Cmin, int &gttu, int S, int x[], int httu[], int &dem);
...
int main()
{
	...
	*/
                    /* vd1
                    {VC, 2,44, 5,26, 6,49,VC,45, 9 },
                    {12, VC,VC,1,48,12,23,33, VC,26 },
                    {31,36, VC, VC,6,48,VC, 8,48, VC },
                    { 43,42,2, VC,35,42,25, VC,16, 5 },
...
//    Input(STP, C);
	...
	Try(1, STP, C, daxet, Cmin, gttu, S, x, httu, dem);
...
void Input(int n, int C[][STP])
{
    ...
    for(i=0; i<n; i++)
        for(j=0; j<n; j++)
        {
            ...
            {
                //random[1->50], random[a->b]: a+rand()%(b-a+1)
                C[i][j] = 1 + rand()%51;
...
void Gan(int httu[], int gttu, int x[], int n)
{
    ...
    {
        httu[i] = x[i];
        printf("%2d ", httu[i]);//in ra tat ca cac hanh trinh co the
    }
    printf("- Gttu: %3d\n\n", gttu);
}
//thu?
			...
			else
			{
				...
				if (g < gttu)
                {
                    ...
                    if(gttu<VC)
                        ...
                        printf(" VC\n");
                    Try(i+1, n, C, daxet, Cmin, gttu, S, x, httu, dem);
                }

			}
			S = S - C[x[i-1]][x[i]];
			daxet[j] = 0;
			printf("-Huy trang thai x[%d]=%d",i,j);
			if(j==n-1)
                printf("\n=>Ket thuc Try(%d)\n\n", i);
            else
                printf(". Tiep tuc Try(%d).j++\n", i);
		}
		else
            printf("j=%d: j da xet!\n", j);
	}
